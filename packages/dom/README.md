[![Nirvarnia](https://raw.githubusercontent.com/nirvarnia/branding/75219792b63f32e002173817be8462706af232ff/dist/banners/banner-02.jpg?sanitize=true)](https://github.com/nirvarnia)

# @nirvarnia/doc

**Source files and build scripts for the `@nirvarnia/doc` package.**

## Documentation

DEPRECATED: This documentation has moved to the /dev repo.

These are instructions to build, test and distribute the @nirvarnia/doc package. See the [package repository](https://www.npmjs.com/package/@nirvarnia/doc/v/0.0.0) or the [website](https://nirvarnia.com/) for installation and API documentation.

- [Requirements](docs/en/requirements.md)
- [Project Filesystem](docs/en/files.md)
- [Building](docs/en/build.md)
- [Testing](docs/en/test.md)
- [Static Analysis](docs/en/lint.md)
- [Releasing](docs/en/release.md)


## Contribute

You can improve this free and open source software project by:

- Reporting bugs
- Suggesting features
- Contributing source code
- Sponsoring our work

Go to the [Nirvarnia Development](https://github.com/nirvarnia/dev) content repository to find out more.

----

Copyright © 2021 Kieran Potts _et al._ \
[MIT License](./LICENSE.txt)
